...
- `workflow_draft` (`draft_type: 'moderation'`) stores both the `moderation_finalized` flag and per-scenario autosave data under `scenario_drafts`. Autosave is debounced (500ms) and uses a read-modify-write pattern to preserve other scenarios' data. `loadScenario()` restores from the draft as a third-priority fallback after backend DB and in-memory Map. See `docs/SCENARIO_AUTOSAVE_AND_VALIDATION.md` for the full draft blob structure.
