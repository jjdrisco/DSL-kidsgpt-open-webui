...
class User(Base):
    ...
    parent_id = Column(String, nullable=True)  # For child accounts, links to parent user
...
class UsersTable:
    ...
    def get_user_by_prolific_pid(self, prolific_pid: str, db: Optional[Session] = None) -> Optional[UserModel]:
        try:
            with get_db_context(db) as db:
    ...
    def update_user_session(self, user_id: str, session_id: str, session_number: int, db: Optional[Session] = None) -> Optional[UserModel]:
        try:
            with get_db_context(db) as db:
