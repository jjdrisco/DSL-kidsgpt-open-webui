...
c.execute("""SELECT scenario_index, is_attention_check, attention_check_passed,
...
# Workflow draft
c.execute("SELECT user_id, section, progress_data, updated_at FROM workflow_draft ORDER BY updated_at DESC LIMIT 3")
drafts = c.fetchall()
print("\n=== Workflow Drafts ===")
for d in drafts:
    dd = dict(d)
