...
describe('Kids Profile', () => {
	...
	beforeEach(() => {
		...
		const EMAIL =
			Cypress.env('INTERVIEWEE_EMAIL') || Cypress.env('TEST_EMAIL') || 'jjdrisco@ucsd.edu';
	...
	it('should be able to add a new child profile', () => {
		...
		cy.get('body').then(($body) => {
			if (
				$body.find('button:contains("Add Profile")').length > 0 ||
	...
	it('should display existing child profiles', () => {
		...
		cy.get('body').then(($body) => {
			...
			if (
				$body.text().includes('Select Your Profile') ||
				$body.text().includes('Profile Information') ||
				$body
					.find('button')
					.filter((i, el) => el.textContent && /Kid|Child|Profile/.test(el.textContent)).length > 0
			) {
				...
				cy.get('body').then(($body2) => {
					...
					if (editButton.length > 0) {
						...
						cy.get('textarea[id="childCharacteristics"]').then(($textarea) => {
							...
							cy.get('textarea[id="childCharacteristics"]')
								.clear()
								.type(currentValue + ' - Updated by test');
						});
	...
	it('should validate required fields when creating a profile', () => {
		...
		cy.get('body').then(($body) => {
			if (
				$body.find(
					'button:contains("Add Profile"), button:contains("Add Your First Child Profile")'
				).length > 0
			) {
				cy.contains('button', /Add.*Profile/i).click();
				cy.wait(1000);
