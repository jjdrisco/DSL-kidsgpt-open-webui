					...
					if ($isLastActiveTab) {
						if ($settings?.notificationEnabled ?? false) {
							new Notification(`${title} • Open WebUI`, {
								body: content,
								icon: `${WEBUI_BASE_URL}/static/whitelist-ai.png`
							});
						}
					}
...
<svelte:head>
	...
	<link crossorigin="anonymous" rel="icon" href="{WEBUI_BASE_URL}/static/whitelist-ai.png" />
...
{#if showConsentModal}
	<ConsentModal
		...
		studyId={$page.url.searchParams.get('STUDY_ID') ||
			localStorage.getItem('prolificStudyId') ||
			$user?.study_id ||
			null}
	/>
{/if}
