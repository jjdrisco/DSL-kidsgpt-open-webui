<script lang="ts">
	...
	$: if (childAge) {
		...
		const age = typeof childAge === 'number' ? childAge : parseInt(String(childAge), 10);

		if (!isNaN(age)) {
			availableFeatures = getAvailableFeatures(age);

			const ageGroup = getAgeGroupFromValue(age);
			ageGroupLabel = ageGroup?.label || `Age ${age}`;
					...
					<div class="flex items-start justify-between">
						<div class="flex-1">
							<div class="flex items-center gap-2 mb-2">
								...
								{#if isAuto}
									<span class="text-yellow-500 text-sm" title="Pre-selected for this age">★</span>
								{/if}
							</div>
