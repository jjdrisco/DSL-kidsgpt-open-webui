<script lang="ts">
	...
	import { isInterfaceModeEnabled, getCurrentChildProfile, currentUserChildProfile } from '$lib/utils/interfaceModes';
	...
	let showAvailableFeatures = false;
	...
	$: (() => {
		const p = $currentUserChildProfile;
		const sf = p?.selected_features ?? [];
		console.log('[MessageInputChild] currentUserChildProfile / features:', {
			hasProfile: !!p,
			profileId: p?.id,
			selected_features: sf,
			selected_features_length: sf.length,
			matchingNames: sf.map((id) => CHILD_FEATURES.find((f) => f.id === id)?.name ?? id)
		});
	})();
					...
					>
						...
						>
							...
							<div class="px-2.5">
								...
								{#if isInterfaceModeEnabled('text_input')}
									{#if suggestions}
								...
								{:else}
									<div class="p-4 text-center text-gray-500 dark:text-gray-400">
										{$i18n.t('Text input is not enabled for your profile.')}
									</div>
								{/if}
							</div>
						</div>
						...
						<div class=" flex justify-between mt-0.5 mb-2.5 mx-0.5 max-w-full" dir="ltr">
							<div class="ml-1 self-end flex items-center flex-1 max-w-[80%] gap-1.5">
								{#if isInterfaceModeEnabled('photo_upload') || ($currentUserChildProfile?.selected_features ?? []).includes('school_assignment') || !$currentUserChildProfile?.selected_interface_modes?.length}
									<InputMenu
										hideNotesAndKnowledge={true}
										bind:files
								...
								{#if ($currentUserChildProfile?.selected_features ?? []).length > 0}
									<Dropdown
										bind:show={showAvailableFeatures}
										on:change={(e) => {
											if (e.detail === false) {
												showAvailableFeatures = false;
											}
										}}
									>
										<Tooltip content={$i18n.t('Available Features')} placement="top">
											<button
												type="button"
												class="bg-transparent hover:bg-gray-100 text-gray-600 dark:text-gray-300 hover:text-gray-700 dark:hover:text-gray-200 dark:hover:bg-gray-800 rounded-full size-8 flex justify-center items-center outline-hidden focus:outline-hidden"
											>
										...
										<div slot="content">
											...
											>
												{#each ($currentUserChildProfile?.selected_features ?? []) as featureId}
													{@const feature = CHILD_FEATURES.find((f) => f.id === featureId)}
													{#if feature}
														<DropdownMenu.Item
															class="flex gap-2 items-center px-3 py-1.5 text-sm cursor-default rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800/50"
														>
															{#if feature.icon}
																<span class="text-base">{feature.icon}</span>
															{/if}
															<div class="line-clamp-1">{feature.name}</div>
														</DropdownMenu.Item>
													{/if}
												{/each}
											</DropdownMenu.Content>
								...
								<div class="self-end flex space-x-1 mr-1 shrink-0 gap-[0.5px]">
									...
									{#if isInterfaceModeEnabled('voice_input') && (!history?.currentId || history.messages[history.currentId]?.done == true) && ($_user?.role === 'admin' || ($_user?.permissions?.chat?.stt ?? true))}
										<!-- {$i18n.t('Record voice')} -->
									...
									{#if isInterfaceModeEnabled('voice_input') && prompt === '' && files.length === 0 && ($_user?.role === 'admin' || ($_user?.permissions?.chat?.call ?? true))}
										<div class=" flex items-center">
											<!-- {$i18n.t('Call')} -->
