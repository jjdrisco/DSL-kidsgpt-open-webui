<script lang="ts">
	...
	import MessageInputChild from '$lib/components/chat/MessageInputChild.svelte';
	...
	const sendMessage = async (
		...
		for (const [_modelIdx, modelId] of selectedModelIds.entries()) {
			console.log('[ChildChat] submitPrompt - Looking for model:', modelId);
			console.log('[ChildChat] submitPrompt - Available models:', $models.length, $models.map((m) => m.id).slice(0, 10));
			const model = $models.filter((m) => m.id === modelId).at(0);

			if (model) {
				console.log('[ChildChat] submitPrompt - Model found:', model.id);
		...
		await Promise.all(
			selectedModelIds.map(async (modelId, _modelIdx) => {
				console.log('[ChildChat] Looking for model:', modelId);
				console.log('[ChildChat] Available models count:', $models.length);
				console.log('[ChildChat] Available model IDs:', $models.map((m) => m.id).slice(0, 20));
				const model = $models.filter((m) => m.id === modelId).at(0);

				if (model) {
					console.log('[ChildChat] Model found:', model.id, model.name);
					...
					console.error('[ChildChat] Model not found:', {
