...
export function getPiagetStageForAge(age: number): PiagetStage | null {
	return PIAGET_STAGES.find((stage) => age >= stage.minAge && age <= stage.maxAge) || null;
}
...
export function validateModesForAge(
	...
	const invalidModes = selectedModeIds.filter(
		(id) => !availableModeIds.includes(id as InterfaceMode['id'])
	);
