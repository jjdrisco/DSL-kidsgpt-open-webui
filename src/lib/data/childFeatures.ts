...
export const AGE_GROUPS: AgeGroup[] = [
	{ id: '6-8', minAge: 6, maxAge: 8, label: 'Ages 6-8', piagetStage: 'preoperational' },
	{
		...
		maxAge: 12,
		label: 'Ages 9-12',
		piagetStage: 'concrete_operational'
	},
	{
		id: '13-15',
		minAge: 13,
		maxAge: 15,
		label: 'Ages 13-15',
		piagetStage: 'formal_operational'
	},
...
export function getRecommendedFeatures(age: number): ChildFeature[] {
	const ageGroup = AGE_GROUPS.find((group) => age >= group.minAge && age <= group.maxAge);

	if (!ageGroup) {
		return [];
	}

	return CHILD_FEATURES.filter((feature) => feature.recommendedFor.includes(ageGroup.id));
}
...
/**
 * Get age group from age string (e.g., "9 years old" -> "9-12")
 */
...
export function validateFeaturesForAge(
	...
	const invalidFeatures = selectedFeatureIds.filter((id) => !availableFeatureIds.includes(id));
